...
export default function Intake() {
  ...
  const submit = async () => {
    ...
    setSubmitting(true);
    try {
      // Coerce empty strings to null for numerics; arrays kept as-is
      const payload: Form = {};
  ...
  return (
    <AppShell>
      <div className="max-w-3xl mx-auto">
        <div className="mb-10">
          <p className="curve-eyebrow mb-2">Revenue Assessment</p>
          <h1 className="font-display text-3xl font-semibold tracking-tight mb-1">{SECTION_TITLES[step]}</h1>
        ...
        <div className="curve-card space-y-6">
          {step === 0 && (
          ...
          {step === 2 && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <NumberField label="Total Annual Revenue" value={form.total_annual_revenue} onChange={(v) => set("total_annual_revenue", v)} min={0} />
              <NumberField label="Average HS Player Fee per year" value={form.avg_hs_player_fee} onChange={(v) => set("avg_hs_player_fee", v)} min={0} />
              <NumberField label="Average Youth Player Fee per year" value={form.avg_youth_player_fee} onChange={(v) => set("avg_youth_player_fee", v)} min={0} />
              <div className="sm:col-span-2">
              ...
              <NumberField label="Total Sponsorship Revenue" value={form.total_sponsorship_revenue} onChange={(v) => set("total_sponsorship_revenue", v)} min={0} />
              <NumberField label="Apparel Revenue" value={form.apparel_revenue} onChange={(v) => set("apparel_revenue", v)} min={0} />
              ...
              {runsEvents && (
                <>
                  ...
                  <NumberField label="Total Annual Event Revenue" value={form.total_event_revenue} onChange={(v) => set("total_event_revenue", v)} min={0} />
                </>
              )}
              <NumberField label="Camps Revenue" value={form.camps_revenue} onChange={(v) => set("camps_revenue", v)} min={0} />
              <NumberField label="Clinics Revenue" value={form.clinics_revenue} onChange={(v) => set("clinics_revenue", v)} min={0} />
              <NumberField label="Lessons Revenue" value={form.lessons_revenue} onChange={(v) => set("lessons_revenue", v)} min={0} />
              <NumberField label="Showcase Revenue" value={form.showcase_revenue} onChange={(v) => set("showcase_revenue", v)} min={0} />
              <NumberField label="Other Add-On Revenue" value={form.other_addon_revenue} onChange={(v) => set("other_addon_revenue", v)} min={0} />
              {isFacility && (
                <NumberField label="Annual Facility Rental Revenue" value={form.facility_rental_revenue} onChange={(v) => set("facility_rental_revenue", v)} min={0} />
              )}
            </div>
          )}
        ...
        <div className="flex justify-between mt-6">
          ...
          {step < totalSteps - 1 ? (
            <Button onClick={() => setStep((s) => Math.min(totalSteps - 1, s + 1))}>Continue</Button>
          ) : (
